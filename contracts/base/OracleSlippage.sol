...
import '@balanceswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@balanceswap/v3-periphery/contracts/libraries/Path.sol';
import '@balanceswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@balanceswap/v3-core/contracts/interfaces/IBalanceSwapV3Pool.sol';
import '@balanceswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IBalanceSwapV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IBalanceSwapV3Pool pool) {
        pool = IBalanceSwapV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IBalanceSwapV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IBalanceSwapV3Pool(pool).slot0();
            }
