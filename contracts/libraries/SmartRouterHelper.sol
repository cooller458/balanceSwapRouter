...
import '@balanceswap/v3-core/contracts/libraries/LowGasSafeMath.sol';
...
library SmartRouterHelper {
    ...
    /// @param deployer The BalanceSwap V3 deployer contract address
    ...
    function getPool(
        ...
        return IBalanceSwapV3Pool(computeAddress(deployer, getPoolKey(tokenA, tokenB, fee)));
    }

    /// @notice Returns the address of a valid BalanceSwap V3 Pool
    /// @param deployer The contract address of the BalanceSwap V3 deployer
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (IBalanceSwapV3Pool pool) {
    ...
    function verifyCallback(address deployer, PoolKey memory poolKey)
        public
        view
        returns (IBalanceSwapV3Pool pool)
    {
        pool = IBalanceSwapV3Pool(computeAddress(deployer, poolKey));
